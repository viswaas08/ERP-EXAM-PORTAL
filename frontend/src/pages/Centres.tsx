...
export function Centres() {
  ...
  return (
    <section className="space-y-5">
      ...
      <Card className="grid gap-3 md:grid-cols-4"><Input placeholder="Centre name" value={name} onChange={(event) => setName(event.target.value)} /><Input placeholder="City" value={city} onChange={(event) => setCity(event.target.value)} /><Select value={state} onChange={(event) => setState(event.target.value)}>{stateOptions.map((item) => <option key={item} value={item}>{item || "Select State"}</option>)}</Select><Button className="bg-secondary" onClick={createCentre}>Create Centre</Button></Card>
