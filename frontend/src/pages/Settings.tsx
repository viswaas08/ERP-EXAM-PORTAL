  ...
  return (
    <section className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="space-y-3"><h2 className="font-semibold">Portal Identity</h2><Input value={org} onChange={(event) => setOrg(event.target.value)} /><Input value={portal} onChange={(event) => setPortal(event.target.value)} /><Select onChange={(event) => setNotice(`Theme setting changed to ${event.target.value}.`)}>{themeOptions.map((item) => <option key={item}>{item}</option>)}</Select></Card>
        ...
        <Card className="space-y-3"><h2 className="font-semibold">Notifications</h2><Input defaultValue="smtp.exam.gov" /><Input defaultValue="notices@exam.gov" /><Select>{popupOptions.map((item) => <option key={item}>{item}</option>)}</Select></Card>
        <Card className="space-y-3"><h2 className="font-semibold">Backup</h2><Input defaultValue="Daily 02:00 AM" /><Select>{retentionOptions.map((item) => <option key={item}>{item}</option>)}</Select><Button onClick={() => setNotice(`${portal} settings saved for ${org}.`)}><Save size={18} /> Save Settings</Button></Card>
      </div>
    </section>
  );
}
