    ...
    <main className="min-h-screen bg-background p-4 md:p-8">
      <div className="mx-auto max-w-6xl space-y-5">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card><Ticket className="mb-3 text-primary" /><h2 className="font-semibold">Hall Ticket</h2><p className="my-3 text-sm text-slate-500">Roll No 26000012, Centre Pune Digital Campus.</p><Button onClick={() => { downloadFile("hall-ticket.txt", "Hall Ticket\nRoll No: 26000012\nCentre: Pune Digital Campus"); setNotice("Hall ticket downloaded."); }}><Download size={18} /> Download PDF</Button></Card>
          <Card><MonitorPlay className="mb-3 text-secondary" /><h2 className="font-semibold">Online Examination</h2><p className="my-3 text-sm text-slate-500">System check and countdown available before exam.</p><div className="flex flex-wrap gap-2"><Button className="bg-secondary" onClick={() => setNotice("System check passed. Browser, keyboard, and timer are ready.")}>System Check</Button><Link to="/exam"><Button>Open Exam Console</Button></Link></div></Card>
          <Card><FileText className="mb-3 text-amber-600" /><h2 className="font-semibold">Result</h2><p className="my-3 text-sm text-slate-500">{resultVisible ? "Marks 146/200, Rank 42, Qualified." : "Score card will appear after result publication."}</p><div className="flex flex-wrap gap-2"><Button onClick={() => { setResultVisible(true); setTimelineIndex(6); setNotice("Result opened and timeline updated."); }}>View Result</Button>{resultVisible && <Button className="bg-secondary" onClick={() => downloadFile("score-card.txt", "Score Card\nMarks: 146/200\nRank: 42\nQualified: Yes")}>Download</Button>}</div></Card>
        </div>
      </div>
    </main>
  );
